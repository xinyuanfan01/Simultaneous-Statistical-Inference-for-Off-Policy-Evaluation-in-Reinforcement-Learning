...
action<-function(x){  
  p=0.5*sigmoid(x[1])+0.5*sigmoid(x[2])
  rbinom(1,1,p)
}


new_state<-function(x,a){
  A=diag(c(2*a-1,1-2*a))
  0.75*A%*%x+MASS::mvrnorm(1,rep(0,2),diag(c(1,1))*4)
}

reward<-function(x,a){
  2*x[1]+x[2]-0.25*(2*a-1)
}
...
V<-function(x,Tot,gamma,B=1000,dimen=2,reward_1=reward,state=new_state,policy=target_policy){
  res=rep(0,B)
  for(b in 1:B){
...
phi_L<-function(x,dimen=2){
  ...
  if(dimen==2){
    ...
    return(
      purrr::map_dbl(1:(L1),~approx(xx, basis_mat[,.], xout = x[1])$y)
    )
  }
}
...
estimated_V<-function(X,A,Y,n,gamma,target_policy,...){
  ...
  sumTi=sum(unlist(lapply(Y,length)))
...
bootstrap_quantile<-function(state_grid,cov_mat,target_policy,Bn=1e4,...){
  ...
  c(0,quantile(do.call(pmax, a1) ,0.95)
  )
}
