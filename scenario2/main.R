# This file replicates Table 1 presented in the main paper with the spline basis.

source("funs.R")
library(RhpcBLASctl) 

param_grid <- data.frame(n = c(30,50,40,50,50,200, 250, 300, 50, 50, 50),
                         Tot = c(50,30,50,40,50,70, 70, 70, 150, 200, 250),
                         L=c(7,8,7,7,8,12, 12, 13, 12, 12, 13)) 
...
load(file="V_real_sequence_s2.rdata")
...
for(iii in 1:nrow(param_grid)){
  ...
  state_grid=generate_grid_from(c(-1,1),n_points = 30)
  ...
  res_para <- foreach(kk_new=1:repeat_time,.combine='rbind')%dopar%{
    tryCatch({
      ...
      #SCB construction
      q1=bootstrap_quantile(state_grid,cov_mat,target_policy) 
